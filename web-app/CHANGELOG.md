...
### v1.12.4 (2026-02-25) - 图片永久保存
**主题：AI 生成图片自动转 Base64 永久保存**

#### ✨ 新功能
- **图片自动永久化**：AI 生成配图后自动下载并转为 Base64，避免 CDN 链接过期失效
- **一键转换**：设置页新增「CDN 图片永久化」功能，可批量将已有 CDN 图片转为 Base64
- **图片统计**：显示当前词本的图片存储情况（CDN 数量 vs 永久保存数量）

#### 🔧 改进
- **图片压缩**：转换时自动压缩（600px，75% 质量），控制存储空间
- **容错处理**：CORS 限制导致转换失败时保留原始 URL

---

### v1.12.3 (2026-02-25) - 配图加载容错
**主题：修复配图加载失败时的显示问题**

#### 🐛 修复
- **配图加载失败**：当外部 CDN 图片 URL 失效时，自动显示关键词占位符，不再显示破损图片
- **新增 WordImage 组件**：统一处理图片加载状态，支持加载中/失败/成功三种状态

#### 🔧 改进
- **全局图片容错**：WordCard、FlashcardMode、MatchMode、WordList 全部使用新组件
- **控制台警告**：图片加载失败时在控制台输出警告信息，便于排查

---

### v1.12.2 (2026-02-13) - 音频与消消乐修复
**主题：修复单词发音无声音、消消乐点击空白**

#### 🐛 修复
- **单词音频播放**：整段播放放入 runAfterUnlock，Chrome 下恢复发音
- **单词消消乐**：使用 Portal 挂载到 body，解决点击后空白页

#### ✨ 优化
- **消消乐按钮**：浅色主题 hover 样式修正

---

### v1.12.1 (2026-02-13) - 闪卡自然拼读与体验
**主题：自然拼读分段展示与分段播放、点击播单词、字号加大**

#### 🔧 改进
- **自然拼读展示**：查看释义时单词按音节分段、双色显示（主色/琥珀色），强化拼读
- **分段播放**：揭示释义时第 1 遍正常读、第 2 遍按音节间隔读、第 3 遍正常读
- **查看释义点击播单词**：在释义页点击空白/卡片区域即播放单词；生/熟悉按钮不误触
- **闪卡字号**：单词与例句在释义状态下再次加大（单词 5xl～7xl，例句 xl～2xl）
- **修复**：选「生」时正确插入队列并自动切到下一张

---

### v1.12.0 (2026-02-13) - 闪卡与单词消消乐
**主题：闪卡样式优化 + 单词消消乐模式**

#### ✨ 新功能
- **单词消消乐**：一屏 5 个词、一局最多 15 词，随机「图片⇄单词」或「释义⇄单词」；答对/答错音效，连对统计与鼓励反馈；词本 ≥5 即可开局
- **图片模式仅用有图词**：消消乐选到图片模式时，只从有图单词中抽词

#### 🔧 改进
- **闪卡默认显示图片**：先显示配图，点击后在同一页揭示单词、释义、例句（不再翻转）
- **闪卡按钮「生/熟悉」**：选「生」隔 4 张再出现，选「熟悉」不再出现；标记后即时更新掌握度并持久化
- **闪卡例句字号**：例句与翻译在闪卡中加大，更易读

---

### v1.11.1 (2026-02-13) - 生图口令优化
**主题：动词/形容词用人物+辅助物品表现，更直观**

#### 🔧 改进
- **生图口令优化**：动词改为「人物+道具」表现动作（如手持/接触相关物品），不再使用小企鹅
- **形容词/副词/抽象词**：统一为「人物或场景+辅助物品」体现含义，一眼能看懂
- **名词**：仍使用 3D 黏土风格，无变化
- Web 应用与 Chrome 插件生图逻辑已同步
...
#### ✨ 新功能
- **Chrome 扩展**：右键菜单添加单词、快捷键（⌘E / Ctrl+E）添加
